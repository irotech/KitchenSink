...
public class Week2 {
    ...
    private URL resource = getClass().getClassLoader().getResource("mooc/oracle/jdk8_lambdas_streams/SonnetI.txt");
...
//    list.stream().forEach(item -> System.out.println(item.toLowerCase()));
    ...
    private void exercise4() throws IOException {
        try (BufferedReader reader = Files.newBufferedReader(
            ...
            System.out.println("Number of line in the file: " + reader.lines().count());
        }
    }
    ...
    private void exercise5() throws IOException {
        try (BufferedReader reader = Files.newBufferedReader(
                Paths.get(resource.getPath()), StandardCharsets.UTF_8)) {
//      List<String> list =
//              reader.lines().filter(Pattern.compile(WORD_REGEXP).asPredicate()).distinct()
//                      .collect(Collectors.toList());
